...
bool CGMNebula::Init(SGMKernelData* pKernelData, SGMConfigData* pConfigData)
{
	int iWidth = 480;
	int iHeight = 270;
	// 由于其他模块需要用到这张距离图，所以必须初始化时就分配好空间
...
bool CGMNebula::UpdateLater(double dDeltaTime)
{
	if (m_rayMarchCamera.valid() && m_cameraTAA.valid() && m_rayMarchCamera->getCullMask())
	{
		...
		bool bFrustum = GM_Viewer->getCamera()->getProjectionMatrixAsFrustum(fLeft, fRight, fBottom, fTop, fZNear, fZFar);
		if (bFrustum)
		{
			...
			if (iShakeCount % 2)
			{
				...
				m_statesetTAA->setTextureAttributeAndModes(m_iUnitColor, m_rayMarchMap_0.get());
				m_statesetTAA->setTextureAttributeAndModes(m_iUnitVelocity, m_vectorMap_0.get());
				m_statesetTAA->setTextureAttributeAndModes(0, m_TAAMap_1.get(), iAttribute);
				m_statesetTAA->setTextureAttributeAndModes(1, m_distanceMap_1.get(), iAttribute);
			...
			else
			{
				...
				m_statesetTAA->setTextureAttributeAndModes(m_iUnitColor, m_rayMarchMap_1.get());
				m_statesetTAA->setTextureAttributeAndModes(m_iUnitVelocity, m_vectorMap_1.get());
				m_statesetTAA->setTextureAttributeAndModes(0, m_TAAMap_0.get(), iAttribute);
				m_statesetTAA->setTextureAttributeAndModes(1, m_distanceMap_0.get(), iAttribute);
				m_cameraTAA->attach(osg::Camera::COLOR_BUFFER0, m_TAAMap_1.get());
				m_cameraTAA->attach(osg::Camera::COLOR_BUFFER1, m_distanceMap_1.get());
				m_cameraTAA->dirtyAttachmentMap();
			...
			osg::Matrixd mMainViewMatrix = GM_Viewer->getCamera()->getViewMatrix();
			osg::Matrixd mMainProjMatrix = GM_Viewer->getCamera()->getProjectionMatrix();
...
void CGMNebula::MakeNebula(float fLength, float fWidth, float fHeight, float fX, float fY, float fZ)
{
	...
	m_2DNoiseTex = _CreateTexture2D(strTexturePath + "2DNoise.jpg", 3);
	m_3DShapeTex_128 = _Load3DShapeNoise();

	osg::ref_ptr<osg::Geometry> pBoxGeom = _MakeBoxGeometry(fLength, fWidth, fHeight);
	...
	SGMVolumeRange sVR;
	sVR.fXMin = fX - fLength * 0.5f;
	sVR.fXMax = fX + fLength * 0.5f;
	sVR.fYMin = fY - fWidth * 0.5f;
	sVR.fYMax = fY + fWidth * 0.5f;
	sVR.fZMin = fZ - fHeight * 0.5f;
	sVR.fZMax = fZ + fHeight * 0.5f;
	...
	m_iUnitRayMarch++;
	m_stateSetRayMarch->setTextureAttributeAndModes(m_iUnitRayMarch, _Load3DErosionNoise());
	...
	GM_Root->addChild(m_nebulaRayMarchTransform.get());
	...
	GM_Viewer->addEventHandler(new NebulaResizeEventHandler(this));
}
...
void CGMNebula::ResizeScreen(const int width, const int height)
{
	...
	if (m_rayMarchCamera.valid())
	{
		m_rayMarchCamera->resize(std::ceil(fRTTRatio*width), std::ceil(fRTTRatio*height));
	}
...
osg::Texture* CGMNebula::_Load3DShapeNoise()
{
	...
	img->setImage(128, 128, 128, GL_R8, GL_RED, GL_UNSIGNED_BYTE, img->data(), osg::Image::NO_DELETE);
...
* @author LiuTao
* @since 2020.06.22
* @param fRadius			半径，单位：米
* @param iLatSegment		纬度分段数，经度分段数是其两倍
* @return Geometry			返回创建的几何体指针
*/
osg::Geometry* CGMNebula::_MakeSphereGeometry(float fRadius, int iLatSegment)
{
...
* @return Geometry			返回创建的几何体指针
*/
osg::Geometry* CGMNebula::_MakeBoxGeometry(float fLength, float fWidth, float fHeight)
{
	osg::Geometry* geom = new osg::Geometry();
	geom->setUseVertexBufferObjects(true);
