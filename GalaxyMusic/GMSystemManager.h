...
private:
	...
	bool							m_bVolumeHiding = false;		//!< 是否开始隐藏音量控件
	int								m_iVolumeCount = 0;				//!< 控制音量控件隐藏的帧数计数器

	int								m_iFrameCount = 0;				//!< 帧数计数器，30帧一循环
	int								m_iRhythmCount = 0;				//!< 节拍计数器，20一循环
	qint64							m_iTimeRhythmStart = 0;			//!< 节拍计数开始时间，单位：s
	qint64							m_iTimeRhythmEnd = 20;			//!< 节拍计数结束时间，单位：s
