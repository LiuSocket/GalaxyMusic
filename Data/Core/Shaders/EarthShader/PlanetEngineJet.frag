...
void main()
{
	...
	vec3 color = mix(vec3(0.0, 0.3, 1.0), vec3(0.7, 1.0, 1.0), mix(alpha, 1, pow(abs(dotNV),50)));
	...
	if(unit > 1e6)
	{
		...
		float tailMask = 1 - 0.7*tailAlpha;
		gl_FragColor = vec4(color, jetAlpha)*tailMask;
	}
	else
	{
		jetAlpha *= clamp((lenV-minDistance)/minDistance, 0, 1);
		gl_FragColor = vec4(color, jetAlpha);
	}
}