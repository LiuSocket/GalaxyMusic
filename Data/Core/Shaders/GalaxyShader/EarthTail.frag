...
const int STEP_NUM = 256;
const int LIGHT_SAMPLE = 4;
const float UNIT = 1.0;
const float LAST_SCLAE = 64;
const float RANGE_MAX = 3;
const float STEP_LENGTH = 1e-6*UNIT;
const float EARTH_RADIUS = 6.378137e-4*UNIT;
const float ATMOS_RADIUS = 7.2e-4*UNIT;
const float TAIL_RADIUS = 2.6e-4*UNIT;
const float TAIL_LENGTH = 2e-2*UNIT;
const float LENGTH_MAX = 1e-2*UNIT;
const float TAIL_ATMOS_RATIO = TAIL_RADIUS / ATMOS_RADIUS;
const float EARTH_ATMOS_RATIO = EARTH_RADIUS / ATMOS_RADIUS;
const float M_PI = 3.141592657;
const float ALPHA_MAX = 0.99;
const float LIGHT_LEN[LIGHT_SAMPLE] = float[](4e-6*UNIT, 8e-6*UNIT, 1.6e-5*UNIT, 3e-5*UNIT);
...
vec2 LenMinMax(vec3 modelEyePos, vec3 modelPixDir)
{
	// cull by earth
	float dotED = dot(-modelEyePos, modelPixDir);
	vec3 modelCorePointM = modelEyePos + dotED*modelPixDir;
	float dstEarth = length(modelCorePointM)/EARTH_RADIUS;
	...
	for(int i = 0 ; i < 8 ; i ++)
	{
		float radiusNow = ATMOS_RADIUS * (0.5 + 0.5*exp2(-float(i)*0.4));
		vec3 tailSpherePos = vec3(0.0, 0.0, EARTH_RADIUS*(0.15+0.7*exp2(-float(i)*0.05)*float(i)));
...
float AtmosDens(vec3 modelStepPos, vec3 modelStepDir)
{
	...
	erosionUVW *= vec3(1.83, 1.83, 0.413);		
	erosionUVW.z -= times*0.005;

	float shapeScale = 0.4+0.6*smoothstep(0.1, 1.0, modelStepDir.z);
	vec3 offsetUVW = texture3D(noiseErosionTex, fract(erosionUVW)).bgr;
	vec3 shapeUVW = modelStepPos/ATMOS_RADIUS;
	shapeUVW *= vec3(2*shapeScale, 2*shapeScale, 0.2);		
	shapeUVW.z -= times*0.021;
	shapeUVW += offsetUVW*0.2;

	vec4 shape4 = texture3D(noiseShapeTex, fract(shapeUVW));
	float densFade = clamp((ratioR-EARTH_ATMOS_RATIO)*15, 0.1, 1.0);
	float texDens = abs(1 - densFade*0.66*shape4.x-0.36*shape4.y-0.18*shape4.z-0.09*shape4.w);

	// fade by ratioR
	float sphereAltFade = clamp(1 - (ratioR - EARTH_ATMOS_RATIO)*9.0, 0, 1);
	float tailAltFade = clamp(1 - (ratioR - EARTH_ATMOS_RATIO)*0.05, 0, 1);
	texDens *= mix(tailAltFade, sphereAltFade, clamp((ratioXY-0.5)*2, 0, 1));
	texDens *= clamp((modelStepDir.z+0.2)*5,0,1);

	// fade by radiusXY
	// the propulsion engine fade
	float fadeR = clamp((TAIL_ATMOS_RATIO - ratioXY)*5, 0, 1);
	// the torque engine fade
	fadeR += clamp(clamp(3-ratioZ, 0, 1)*100*(ratioXY - TAIL_ATMOS_RATIO)*(1.5*TAIL_ATMOS_RATIO - ratioXY), 0, 1);
	texDens *= mix(1, fadeR, clamp((ratioR-(1+EARTH_ATMOS_RATIO)*0.5)*2,0,1));
	return texDens;
}
...
void Tail(commonParam cP, inout vec4 tailColor, inout float lenTail)
{
	...
	for(int j = int(jStart); all(bvec2(tailC.a < ALPHA_MAX, j < jEnd)); j++)
	{
		...
		float lenStartStep = lenMinJ + cP.noiseD*lenUnitJ*0.02;
		...
		for(int i = int(startStepNum);all(bvec3(tailC.a<ALPHA_MAX, lenStep<(lenRange*0.999), lenS<(cP.lenMin+lenD))); i++)
		{
			...
			{
				// earth shadow
				vec3 shadow = 1-(1-exp2(min(0, length(modelStepPos.yz)/EARTH_RADIUS-1)*vec3(40, 80, 120)))*step(0, modelStepPos.x);
				// ambient color
				vec3 ambient = 1 - smoothstep(vec3(-0.2,-0.4,-0.6), vec3(0.3,0.2,0.1), vec3(modelStepDir.x));
				float ratioR = length(modelStepPos) / EARTH_RADIUS;
				float ambientFade = (1-exp2(-atmosDens*10))*exp2(min(0, 1-ratioR)*2);
				// diffuse color
				float densLight = atmosDens;
				for(int k = 0; k < LIGHT_SAMPLE; k++)
				{
					float lenK = (1+0.01*cP.noiseD)*LIGHT_LEN[k];
					vec3 modelStepPosK = modelStepPos + vec3(lenK, 0, 0);
					vec3 modelStepDirK = normalize(modelStepPosK);
					float atmosDensK = AtmosDens(modelStepPosK, modelStepDirK);
					densLight += atmosDensK;
				}
				float beersLawOut = Dens2Bright(densLight);
				vec3 diffuse = cP.scattering*beersLawOut;
				// all color
				vec3 chrome = (vec3(0.0, 0.01, 0.02) + vec3(0.1, 0.15, 0.25)*shadow*ambient)*ambientFade
					+ vec3(4.5, 5.5, 6.5)*shadow*diffuse;

				float tailA = 1 - exp2(-atmosDens*0.15);
	...
	float tailAlpha = tailVisible*tailC.a;
...
void main() 
{
	...
	// pixel direction in model space
	float cosVL = dot(modelPixDir, vec3(-1,0,0));
	const vec3 gBack = -vec3(0.0,0.15,0.3);
	vec3 backScattering = (1-gBack*gBack)/(4*M_PI*pow(1+gBack*gBack-2*gBack*cosVL,vec3(1.5)));
	const vec3 gForward = vec3(0.91, 0.88, 0.8);
	vec3 forwardScattering = (1-gForward*gForward)/(4*M_PI*pow(1+gForward*gForward-2*gForward*cosVL,vec3(1.5)));
	float pR = 0.375*(1+cosVL*cosVL);
	vec3 scattering = 0.1*(backScattering + forwardScattering) + pR;

	float noiseD = texture2D(blueNoiseTex, gl_FragCoord.xy*screenSize.z/128.0).r;
	// bounding shape
	vec2 lenMinMax = LenMinMax(modelEyePos, modelPixDir);
